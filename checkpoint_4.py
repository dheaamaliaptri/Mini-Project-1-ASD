...
class daftar_buku:
    ...
    def fibonacci_search_judul(self, judul):
        if self.head is None:
        ...
        current = self.head
        while current:
            arr_buku.append(current.data)
            current = current.next

        def normalize(text):
            return text.lower()  # Agar pengguna tetap dapat melakukan pencarian walau tidak menggunakan kapital pada huruf pertama
        ...
        def fibonacci_search(arr, x):
            ...
            while fibM > 1:
                ...
                if normalize(arr[i].judul) == normalize(x):
                    return i
            ...
            if fibMMm1 and normalize(arr[offset + 1].judul) == normalize(x):
                return offset + 1

            return None

        result_index = fibonacci_search(arr_buku, judul.lower())
        if result_index is not None:
            return arr_buku[result_index]
        else:
            print("Buku dengan judul '{}' tidak ditemukan.".format(judul))
            return None

    def fibonacci_search_penulis(self, penulis):
        if self.head is None:
            print("Daftar buku kosong.")
            return None
        ...
        def fibonacci_search(arr, x):
            ...
            while fibM > 1:
                ...
                if normalize(arr[i].penulis) == normalize(x):
                    return i
            ...
            if fibMMm1 and normalize(arr[offset + 1].penulis) == normalize(x):
                return offset + 1

            return None

        result_index = fibonacci_search(arr_buku, penulis)

        if result_index is not None:
            return arr_buku[result_index]
        else:
            print("Buku dengan penulis '{}' tidak ditemukan.".format(penulis))
            return None
    ...
    """)
        ...
        while True:
            ...
            if pilihan == "1":
                ...
                while True:
                    os.system("cls")
                    print("\nIngin melakukan pencarian apa?:")
                    print("1. Judul Buku")
                    print("2. Nama Penulis")

                    sub_pilihan = input("\nPilih opsi yang Anda inginkan (1/2), atau 't' untuk kembali: ")
                    ...
                    if sub_pilihan == "1":
                        while True:
                            ...
                            hasil_pencarian = daftar_buku.fibonacci_search_judul(judul)
                            if hasil_pencarian:
                                print("\nBuku ditemukan:")
                                print(hasil_pencarian)
                            else:
                                print("\nBuku tidak ditemukan.")
                            lagi = input("\nApakah Anda ingin melakukan pencarian lagi? (y/t): ")
                            os.system("cls")
                            if lagi.lower() != "y":
                                break
                        ...
                        while True:
                            nama = input("Masukkan nama penulis yang ingin dicari: ")
                            hasil_pencarian = daftar_buku.fibonacci_search_penulis(nama)
                            if hasil_pencarian:
                                print("\nPenulis ditemukan:")
                                print(hasil_pencarian)
                            else:
